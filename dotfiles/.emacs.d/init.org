#+options: toc:2
#+startup: overview
#+title: Emacs 30.2 Configuration
#+author: Gabriele Beltramo

*Important* Need to
- use a bundle obained with the ~p -m dotsys.emacs -p~ tool.
- byte compile packages with ~M-x byte-compile-file~
...
#+begin_src emacs-lisp :tangle yes
  (defun gbel/set-exec-path-and-PATH ()
    "Sync exec-path and PATH from a canonical list of directories."
    (let ((path-dirs '("/home/gabri/bin"
  		     "/home/gabri/.venv/bin"
  		     "/home/gabri/.local/bin"
  		     "/home/gabri/Lang/go/bin"
  		     "/home/gabri/Lang/zig/bin")))
      (setenv "PATH" (concat (string-join path-dirs ":") ":" (getenv "PATH")))
      (setq exec-path (append path-dirs
                              (seq-filter #'stringp exec-path)))
    )
  )
  (gbel/set-exec-path-and-PATH)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (add-to-list 'default-frame-alist '(undecorated . t)) ;; Remove title bar / window decorations
  (setq scroll-conservatively 101)   ;; Scroll just enough text to bring point into view, even if you move far away
#+end_src
...
Need to install fonts from all-the-icons, which is special and requires
moving ~.data/~ to ~.emacs.d/elisp/~

Link: https://github.com/jaypei/emacs-neotree.git and https://github.com/domtronn/all-the-icons.el.git

#+begin_src emacs-lisp :tangle yes 
  ;; Tree view
  ;; Note: need to run all-the-icons-install-fonts and then "$ fc-cache -f -v" in a shell
...
** C and C++
See https://olddeuteronomy.github.io/post/cpp-programming-in-emacs/
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(go-mode . ("~/Lang/go/bin/gopls"))))

  (add-hook 'go-mode-hook
            (lambda () (add-hook 'before-save-hook 'eglot-format nil 'local)))
#+end_src

** Python
Configure ~python-mode~ for ~.py~ files with

#+begin_src emacs-lisp :tangle yes
  ;; --- python-mode ---

  ;; --- start ruff-format.el ---
  ;; See https://github.com/JoshHayes/emacs-ruff-format/blob/main/ruff-format.el
  ;; Note: reformatter is a dependency of some other package I'm already using
  (require 'reformatter)
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(python-base-mode . ("/home/gabri/.venv/bin/ruff" "server"))))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(zig-mode . ("~/Lang/zig/bin/zls"))))
#+end_src

* TRAMP
Transparent Remote Access, Multiple Protocols

Also add ~ControlMaster~ to your ~.ssh/config~
...
#+begin_src emacs-lisp :tangle yes
  ...
  (global-set-key (kbd "<f10>") 'project-query-replace-regexp)
