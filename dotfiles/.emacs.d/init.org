...
#+begin_src emacs-lisp :tangle yes
  ...
  (reformatter-define ruff-format
    ...
    :args (let ((cfg (locate-dominating-file (or (buffer-file-name) default-directory) "ruff.toml")))
          (append
            (when cfg (list "--config" cfg))
            (list "format" "--stdin-filename" (or (buffer-file-name) input-file))))
    :lighter " RuffFmt"
    :group 'ruff-format)
  ...
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(python-base-mode . ("/home/gabri/.venv/bin/ty" "server"))))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (global-set-key (kbd "<f12>") 'eglot-format)
