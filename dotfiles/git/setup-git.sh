...
git config --global alias.g "log --date=format:'%d/%m/%y %H:%M' --graph --pretty=format:'%C(auto, yellow)%h%Creset%C(auto, green)%d%Creset %C(auto, blue)%an%Creset %ad %C(auto, green)|%Creset %s'"
git config --global alias.gv "log --graph --stat --date=format:'%d/%m/%y %H:%M' --pretty=format:'%C(auto, yellow)%h%Creset%C(auto, green)%d%Creset %C(auto, blue)%an%Creset %ad %C(auto, green)%n%CresetSubject: %s%n%b%n'"
